  ...
  class Updater
    ...
    def call
      ...
      repositories.each do |repository|
        ...
        logger.info { "  BitbucketHook: Redmine repository updated: #{repository.identifier} (Git: #{time_diff_milli(tg1, tg2)}ms, Redmine: #{time_diff_milli(tr1, tr2)}ms)" }
      end
    end
    ...
    def exec(command, directory)
      logger.debug { "  BitbucketHook: Executing command: '#{command}'" }

      # Get a path to a temp file
      logfile = Tempfile.new("bitbucket_hook_exec")
      logfile.close
      ...
      if success
        logger.debug { "  BitbucketHook: Command output: #{output_from_command.inspect}" }
      else
        logger.error { "  BitbucketHook: Command '#{command}' didn't exit properly. Full output: #{output_from_command.inspect}" }
      end
    ...
    def find_repositories
      ...
      if params.key?(:repository_id)
        ...
        if param_repo.nil? || param_repo.length == 0
          logger.info {
            "BitbucketHook: The repository '#{params[:repository_id]}' isn't " \
    ...
    # not supplied, assume the Bitbucket repository name is the same as the project
    # identifier.
    def get_identifier
