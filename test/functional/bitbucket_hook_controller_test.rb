...
class BitbucketHookControllerTest < ActionController::TestCase
  ...
  def setup
    ...
    BitbucketHook::Updater.any_instance.expects(:system).never
  ...
  def test_should_render_response_from_bitbucket_hook_when_done
    BitbucketHook::Updater.any_instance.expects(:update_repository).returns(true)
    do_post
    assert_response :success
    assert_match "BitbucketHook: Redmine repository updated", @response.body
  end

  def test_should_render_error_message
    BitbucketHook::Updater
      .any_instance
  ...
  def test_exec_should_log_output_from_git_as_debug_when_things_go_well
    BitbucketHook::Updater.any_instance.expects(:system).at_least(1).returns(true)
    @controller.logger.expects(:debug).at_least(1)
    do_post
  end

  def test_exec_should_log_output_from_git_as_error_when_things_go_sour
    BitbucketHook::Updater.any_instance.expects(:system).at_least(1).returns(false)
