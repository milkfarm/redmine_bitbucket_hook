...
class BitbucketHookUpdaterTest < Minitest::Test
  def project
  ...
  def build_updater(payload, options = {})
    updater = BitbucketHook::Updater.new(payload, options)
  ...
  def test_raises_type_error_if_project_has_no_repository
    assert_raises TypeError do
      project = mock("project", :to_s => "My Project", :identifier => "bitbucket")
  ...
  def test_logs_if_a_logger_is_given
    updater = BitbucketHook::Updater.new(payload)
    updater.stubs(:exec).returns(true)
  ...
  def test_logs_if_a_message_logger_is_given
    ...
    logger = BitbucketHook::MessageLogger.new
    updater.logger = logger
